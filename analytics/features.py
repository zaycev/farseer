...
#14907
#from analytics.features import tfidf_vec

# def update_tfidf():
# 	docs = am.Sample.objects.values("id").all()
# 	tokens = {token.id:token for token in tm.Token.objects.all()}
# 
# 
# 	DSIZE = docs.all()
# 	handled = 0
...
def compute_vec(cursor, did, corpus_size, token_dict):
	cursor.execute("SELECT token_id,field,freq FROM "
				   "analytics_term WHERE sample_id=%s;", (did,))
	...
	nterms = [(token_dict[tid].ind, tfidf / doc_len) for tid, tfidf in terms]
	return nterms
