...
#				response_raw.tp = tp_raw
			...
			# store data
			response.fb_shr = json.loads(fb_raw).get("shares", 0)
			response.fb_com = json.loads(fb_raw).get("comments", 0)
			response.tw = json.loads(tw_raw).get("count", 0)
#			if tp_raw:
#				response.tp_all = json.loads(tp_raw).get("response",{"all":0}).get("all", 0)
#				response.tp_inf = json.loads(tp_raw).get("response",{"influential":0}).get("influential",0)
